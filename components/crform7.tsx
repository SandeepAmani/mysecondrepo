...
//   const initialFormState = Object.fromEntries(crFormConfig.map(f => [f.key, '']));
...
//   const validateFields = (fields: CRFormField[]) => {
...
//     if (!validateFields(crFormConfig)) return;
//     setLoading(true);
//     try {
...
//         if (field.type === 'calendar' && isDate(val)) {
//           val = val.toISOString();
//         }
//         acc[field.key] = val;
//         return acc;
...
//       case 'calendar':
//         return <Calendar value={value ? new Date(value) :null} onChange={e => handleChange(field.key, e.value)} showIcon showTime showSeconds hideOnDateTimeSelect hourFormat="24" minDate={new Date()} className={`w-full ${error ? 'p-invalid' : ''}`} dateFormat="dd-mm-yy" />;
//     }
//   };
...
//               )}
//               {loading && i === steps.length - 1 && <ProgressSpinner style={{ width: '25px', height: '25px' }} strokeWidth="6" />}
//             </div>
//           </AccordionTab>
//         ))}
//       </Accordion>
